...
class ChainedQuizSection {
	function add($vars) {
		...
		// sort order
		$sort_order = $wpdb->get_var($wpdb->prepare("SELECT MAX(sort_order) FROM ".CHAINED_SECTIONS."
			WHERE quiz_id=%d", $vars['quiz_id']));
		$sort_order++;

		$result = $wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_SECTIONS." SET
			quiz_id=%d, description=%s, title=%s, sort_order=%d",
			intval($vars['quiz_id']), $vars['description'], $vars['title'],
			$sort_order));
	...
	function save($vars, $id) {
		...
		$result = $wpdb->query($wpdb->prepare("UPDATE ".CHAINED_SECTIONS." SET
			description=%s, title=%s WHERE id=%d",
			$vars['description'], $vars['title'], $id));
	...
	function delete($id) {
		...
		// delete section
		$result = $wpdb->query($wpdb->prepare("DELETE FROM ".CHAINED_SECTIONS." WHERE id=%d", $id));
  ...
  function display_choices($section, $choices) {
		/*
  	   $autocontinue = $output = '';
  ...
  function calculate_points($section, $answer) {
