<div class="wrap">
	<h1><?php printf(__('Manage Sections in %s', 'chained'), stripslashes($quiz->title));?> </h1>
	...
	<div class="postbox-container" style="width:73%;margin-right:2%;">
		...
		<p><a href="admin.php?page=chainedquiz_sections&action=add&quiz_id=<?php echo $quiz->id?>"><?php _e('Click here to add a new section', 'chained')?></a></p>
		<?php if(count($sections)):?>
			<table class="widefat">
				<thead>
				<tr>
					<th>#</th>
					<th><?php _e('ID', 'chained')?></th>
					<th><?php _e('Section', 'chained')?></th>
					<th><?php _e('Edit / Delete', 'chained')?></th>
				</tr>
				</thead>
				<tbody>
				<?php foreach($sections as $cnt=>$section):
					$class = ('alternate' == @$class) ? '' : 'alternate';?>
					<tr class="<?php echo $class?>">
						<td><?php if($count > 1 and $cnt):?>
							<a href="admin.php?page=chainedquiz_sections&quiz_id=<?php echo $quiz->id?>&move=<?php echo $section->id?>&dir=up"><img src="<?php echo CHAINED_URL."/img/arrow-up.png"?>" alt="<?php _e('Move Up', 'chained')?>" border="0"></a>
						<?php else:?>&nbsp;<?php endif;?>
						<?php if($count > $cnt+1):?>
							<a href="admin.php?page=chainedquiz_sections&quiz_id=<?php echo $quiz->id?>&move=<?php echo $section->id?>&dir=down">
								<img src="<?php echo CHAINED_URL."/img/arrow-down.png"?>" alt="<?php _e('Move Down', 'chained')?>" border="0"></a>
						<?php else:?>&nbsp;<?php endif;?></td>
						<td><?php echo $section->id?></td>
						<td><?php echo $section->title?></td>
						<td><a href="admin.php?page=chainedquiz_sections&action=edit&id=<?php echo $section->id?>"><?php _e('Edit', 'chained')?></a> | <a href="#" onclick="chainedConfirmDelete(<?php echo $section->id?>);return false;"><?php _e('Delete', 'chained')?></a></td>
					</tr>
				<?php endforeach;?>
				</tbody>
			</table>
...
function chainedConfirmDelete(qid) {
	if(confirm("<?php _e('Are you sure?', 'chained')?>")) {
		window.location = 'admin.php?page=chainedquiz_sections&quiz_id=<?php echo $quiz->id?>&del=1&id='+qid;
	}
}
</script>
