...
class ChainedQuizQuestions {
	...
	static function add_question() {
		...
		$allsections = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".CHAINED_SECTIONS." WHERE quiz_id=%d", $quiz->id));
	...
	static function edit_question() {
		...
		// select the quiz, sections, section and question
		$question = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".CHAINED_QUESTIONS." WHERE id=%d", intval($_GET['id'])));
		$section = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".CHAINED_SECTIONS." WHERE id=%d", $question->section_id));
		$quiz = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".CHAINED_QUIZZES." WHERE id=%d", $question->quiz_id));
		// all the sections for that quiz
		$allsections = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".CHAINED_SECTIONS." WHERE quiz_id=%d", $question->quiz_id));
	...
	static function list_questions() {
		...
		$quiz = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".CHAINED_QUIZZES." WHERE id=%d", intval($_GET['quiz_id'])));
		$questions = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".CHAINED_QUESTIONS." WHERE quiz_id=%d ORDER BY section_id, sort_order, id", intval($_GET['quiz_id'])));
