		...
		<form method="post" onsubmit="return chainedQuizValidate(this);">
			...
			<p><label><?php _e('Section title', 'chained')?></label>
				<select name="section_id">
					<?php if(!empty($allsections) and count($allsections)):

						foreach($allsections as $single_section):
							echo '<option value="'.$single_section->id.'"';
							if ($question->section_id == $single_section->id):
								echo 'selected';
							endif;
			...
			<p><label><?php _e('Question contents', 'chained')?></label>
				 <?php echo wp_editor(stripslashes(@$question->question), 'question')?></p>
			<p><label><?php _e('Question type:', 'chained')?></label>
				<select name="qtype" onchange="this.value == 'radio' ? jQuery('#chainedAutoContinue').show() : jQuery('#chainedAutoContinue').hide();">
			...
			<?php _e('Label before the comments field:', 'chained');?>
			<input type="text" name="accept_comments_label" size="30" value="<?php echo empty($question->accept_comments_label) ? __('Your comments:', 'chained') : stripslashes(@$question->accept_comments_label);?>"></p>
