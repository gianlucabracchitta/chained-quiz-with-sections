<div class="wrap">
	...
	<div class="postbox-container" style="width:73%;margin-right:2%;">
		...
		<form method="post" onsubmit="return validateChainedQuiz(this);">
			...
			-->
				<li>{{points}} <?php _e('- Total points collected', 'chained')?></li>
				<?php if(!empty($sections)): ?>
					<?php foreach($sections as $section):?>
							<li>{{section-<?php echo($section->id); ?>-title}}<?php _e('- The title of the section: '.$section->title, 'chained')?>
							<li>{{section-<?php echo($section->id); ?>-description}} <?php _e('- The description of the section:'.$section->title, 'chained')?></li>
							<li>{{section-<?php echo($section->id); ?>-points}} <?php _e('- The total points collected for section:'.$section->title, 'chained')?></li>
							<li>{{result-section-<?php echo($section->id); ?>-title}} <?php  _e('- The result (grade) title for the section: '.$section->title, 'chained')?></li>
							<li>{{result-section-<?php echo($section->id); ?>-text}} <?php  _e('- The result (grade) text/description for the section: '.$section->title, 'chained')?></li>
					<?php endforeach?>
				<?php else: ?>
				<li><b>After a new quiz has been created, insert at least one section in order to show the section's results</b></li>
			<?php endif; ?>
			...
			<p>
				<input type="checkbox" name="email_required" value="1" <?php if(!empty($quiz->email_required)) echo 'checked';?>> <?php _e('Providing email address is required to do the quiz.', 'chained');?></p>
				<p><input type="checkbox" name="show_sectiononfrontend" value="1" <?php if(!empty($quiz->show_sectiononfrontend)) echo 'checked'?>> <?php _e('Show section title on the front-end page of the quiz', 'chained');?></p>
			</div>
