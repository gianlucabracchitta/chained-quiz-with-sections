...
<form method="post" id="chained-quiz-form-<?php echo $quiz->id?>">
	<div class="chained-quiz-area" id="chained-quiz-wrap-<?php echo $quiz->id?>">
		<?php //if(!empty($quiz->email_user) and !is_user_logged_in()):
			if(!empty($quiz->email_required) and !is_user_logged_in()):
			?>
			<div class="chained-quiz-email">
				<p><label><?php _e('Il tuo indirizzo email:', 'chained');?></label> <input type="text" name="chained_email" value="<?php echo @$_POST['chained_email']?>" id="chainedUserEmail" class="chained-quiz-email <?php if(!empty($quiz->email_required)) echo 'required';?>"></p>
			</div>
		<?php endif;?>
		...
		<div class="chained-quiz-comments" id="chained-quiz-comments-<?php echo $question->id?>">
				...
				<!-- <input class='chained-quiz-frontend chained-quiz-comment' type='text' name='comments'> -->
				 <textarea class='chained-quiz-frontend chained-quiz-comment' rows="3" cols="20" name='comments'></textarea>
		</div>
		<?php endif;?>

		<div class="chained-quiz-action" style='display:<?php echo ($question->autocontinue and $question->qtype == 'radio') ? 'none' : 'block';?>'>
			<input type="button" class="green-cta-btn" style="float:left;" id="chained-quiz-action-<?php echo $quiz->id?>" value="<?php echo empty($ui['go_ahead_value']) ?__('Go ahead', 'chained') : $ui['go_ahead_value'] ?>" onclick="chainedQuiz.goon(<?php echo $quiz->id?>, '<?php echo admin_url('admin-ajax.php')?>');" disabled="true">
	...
	<input type="hidden" name="section_id" value="<?php echo $question->section_id?>">
