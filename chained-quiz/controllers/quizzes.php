...
You achieved {{points}} points from {{questions}} questions.', 'chained');
	...
	static function edit_quiz() {
	  ...
	  $output = stripslashes($quiz->output);
		$sections = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".CHAINED_SECTIONS." WHERE quiz_id=%d", intval($_GET['id'])));
		...
		if(!empty($_COOKIE['chained_completion_id'.$quiz->id])) {
			...
			if($exists) {
				$wpdb->query($wpdb->prepare("UPDATE ".CHAINED_USER_ANSWERS." SET
					answer=%s, section_id=%d, points=%f, comments=%s, is_correct=%d
					WHERE quiz_id=%d AND completion_id=%d AND question_id=%d",
					$answer, $section_id, $points, $comments, $correct_var, $quiz->id, intval($_COOKIE['chained_completion_id'.$quiz->id]), $question->id));
			}
			else {
				$wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_USER_ANSWERS." SET
					quiz_id=%d, section_id=%d, completion_id=%d, question_id=%d, answer=%s, points=%f, comments=%s, is_correct=%d",
					$quiz->id, $section_id, intval($_COOKIE['chained_completion_id'.$quiz->id]), $question->id, $answer, $points, $comments, $correct_var));
			}
