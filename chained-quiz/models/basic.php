...
class ChainedQuiz {
   static function install($update = false) {
    ...
    // sections
   	if($wpdb->get_var("SHOW TABLES LIKE '".CHAINED_SECTIONS."'") != CHAINED_SECTIONS) {
			$sql = "CREATE TABLE `" . CHAINED_SECTIONS . "` (
				  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    chainedquiz_add_db_fields(array(
      ...
      array("name" => 'show_sectiononfrontend', 'type' => "TINYINT UNSIGNED NOT NULL DEFAULT 0"),
	  ), CHAINED_QUIZZES);
	  ...
	  update_option('chainedquiz_version', "0.1.0");
	  // exit;
   }
   ...
   static function menu() {
   	...
   	add_menu_page(__('Laura\'s Chained Quiz', 'chained'), __('Laura\'s Chained Quiz', 'chained'), $chained_caps, "chained_quizzes",
      ...
      add_submenu_page(NULL, __('Chained Quiz Sections', 'chained'), __('Chained Quiz Sections', 'chained'), $chained_caps,
     		'chainedquiz_sections', array('ChainedQuizSections','manage'));
    ...
    define( 'CHAINED_SECTIONS', $wpdb->prefix. "chained_sections");
