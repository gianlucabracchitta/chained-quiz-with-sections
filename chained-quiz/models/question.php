...
class ChainedQuizQuestion {
	function add($vars) {
		...
		//find section title to store
		$section = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".CHAINED_SECTIONS." WHERE id=%d", intval($vars['section_id'])));
		...
		$result = $wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_QUESTIONS." SET
			quiz_id=%d, section_id=%d, section_title=%s, question=%s, qtype=%s, `rank`=%d, title=%s, autocontinue=%d, sort_order=%d,
			accept_comments=%d, accept_comments_label=%s",
			intval($vars['quiz_id']), $section->id, $section->title, $vars['question'], $vars['qtype'], intval(@$vars['rank']), $vars['title'],
	...
	function save($vars, $id) {
		...
		$result = $wpdb->query($wpdb->prepare("UPDATE ".CHAINED_QUESTIONS." SET
			section_id=%d, section_title=%s, question=%s, qtype=%s, title=%s, autocontinue=%d, accept_comments=%d, accept_comments_label=%s WHERE id=%d",
			$section->id, $section->title, $vars['question'], $vars['qtype'], $vars['title'], intval(@$vars['autocontinue']),
	...
	function save_choices($vars, $id) {
		...
		foreach($choices as $choice) {
			...
			// else update
			$wpdb->query($wpdb->prepare("UPDATE ".CHAINED_CHOICES." SET section_id=%d,
				choice=%s, points=%s, is_correct=%d, goto=%s WHERE id=%d",
				$section_id, $_POST['answer'.$choice->id], $_POST['points'.$choice->id],
		...
		foreach($_POST['answers'] as $answer) {
			...
			$wpdb->query( $wpdb->prepare("INSERT INTO ".CHAINED_CHOICES." SET
				question_id=%d, section_id=%d, choice=%s, points=%s, is_correct=%d, goto=%s, quiz_id=%d",
				$id, $section_id, $answer, $_POST['points'][($counter-2)], $correct, $_POST['goto'][($counter-2)],
	...
	function display_question($question) {
		...
		if ($quiz->show_sectiononfrontend >0){
			$sectiontitle = '<h4 class="section_title">'.$question->section_title.'</h4>';
		} else {
			$sectiontitle = '';
		}
		...
		$content = wpautop($sectiontitle.$content);
