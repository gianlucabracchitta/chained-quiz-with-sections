...
class ChainedQuizQuiz {
    ...
    $admin_email = sanitize_text_field($vars['admin_email']); // don't use sanitize_email here, we can use multiple email addresses
		$show_sectiononfrontend = empty($_POST['show_sectiononfrontend']) ? 0 : 1;
    $email_required = empty($_POST['email_required']) ? 0 : 1;
		...
		$result = $wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_QUIZZES." SET
			...
			save_source_url=%d, set_email_output=%d, email_output=%s, admin_email=%s, email_required=%d, show_sectiononfrontend=%d",
			$vars['title'], $vars['output'], $email_admin, $email_user, $require_login, $times_to_take,
			$save_source_url, $set_email_output, $email_output, $admin_email, $email_required, $show_sectiononfrontend));
	...
	function save($vars, $id) {
      ...
      $admin_email = sanitize_text_field($vars['admin_email']);
		...
		$result = $wpdb->query($wpdb->prepare("UPDATE ".CHAINED_QUIZZES." SET
			...
			$email_required, $show_sectiononfrontend, $id));
	...
	function delete($id) {
		...
		// delete sections
		$wpdb->query($wpdb->prepare("DELETE FROM ".CHAINED_SECTIONS." WHERE quiz_id=%d", $id));
	...
	function finalize($quiz, $points) {
	    ...
	    $completion_id = empty($_COOKIE['chained_completion_id'.$quiz->id]) ? 0 : intval($_COOKIE['chained_completion_id'.$quiz->id]);
		 ...
		 $resultspersection = $_result->calculatepersection($quiz, $completion_id);


		 // select all sections and points per section

			$allsections = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".CHAINED_SECTIONS."
			WHERE quiz_id =%d", $quiz->id));
		 ...
		 // sections
		 foreach ($resultspersection as $rps){
			 $output = str_replace('{{section-'.$rps->section_id.'-title}}', stripslashes(@$rps->section_title), $output);
			 $output = str_replace('{{section-'.$rps->section_id.'-points}}', $rps->total_points, $output);
			 $output = str_replace('{{result-section-'.$rps->section_id.'-title}}', stripslashes(@$rps->result_title), $output);
			 $output = str_replace('{{result-section-'.$rps->section_id.'-text}}', stripslashes(@$rps->result_description), $output);
		 }
		 ...
		 foreach ($resultspersection as $rps){
			 $email_output = str_replace('{{section-'.$rps->section_id.'-title}}', stripslashes(@$rps->section_title), $output);
			 $email_output = str_replace('{{section-'.$rps->section_id.'-points}}', $rps->total_points, $output);
			 $email_output = str_replace('{{result-section-'.$rps->section_id.'-title}}', stripslashes(@$rps->result_title), $output);
			 $email_output = str_replace('{{result-section-'.$rps->section_id.'-text}}', stripslashes(@$rps->result_description), $output);
		 }
   ...
   function send_emails($quiz, $output) {
   	...
   	if(!empty($quiz->email_user)) {
   		...
   		$message='<html><head><title>$subject</title>
			...
			//not sending email to the user...not automatically
	...
	function answers_table($completion_id) {
		...
		$answers = $wpdb->get_results($wpdb->prepare("SELECT tUA.*, tC.choice as choice, tC.is_correct as choice_correct,
			tQ.question as question, tQ.qtype as qtype, tS.title as sectiontitle
			FROM ".CHAINED_USER_ANSWERS." tUA
			JOIN ".CHAINED_QUESTIONS." tQ ON tQ.id = tUA.question_id
			JOIN ".CHAINED_SECTIONS." tS ON tS.id = tUA.section_id
		...
		$output = '<table class="chained-quiz-answers"><tr><th>'.__('Section', 'chained').'</th>
		...
		foreach($answers as $answer) {
			...
			$output .= '<tr><td>'.$answer->sectiontitle.'</td><td>';
			$output .= '<td>'.$_question->display_question($answer).'</td><td>';
			$output .= $user_answer;
